...
class EditCategoriesWidget extends StatelessWidget {
          ...
          children: [
            ...
            if (categories.entries.isNotEmpty)
              ReorderableListView(
                ...
                children: [
                  for ((int, WorkCategory) category
                      in categories.entries.indexed)
                    ListTile(
  ...
  void _showNewCategoryDialog(
    ...
    TextEditingController controller = TextEditingController(
      text: existingCategory?.displayName,
    );
            ...
            ElevatedButton(
              onPressed: () {
                if (formKey.currentState!.validate()) {
                  WorkCategories categories = Provider.of<WorkCategories>(
                    context,
                    listen: false,
                  );
                  if (existingCategory != null) {
                    String oldName = existingCategory.displayName;
                    categories.updateEntry(
                      existingCategory,
              ...
              child: Text(
                (existingCategory == null ? loc?.add : loc?.save) ??
                    '<add/save>',
              ),
            ),
          ],
